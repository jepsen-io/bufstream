(ns jepsen.bufstream.db.watchdog
  "Bufstream likes to kill itself, which means we have to restart it. That
  opens up the possibility of all kinds of race conditions. This namespace
  provides a watchdog which runs in a thread for the duration of the test. It
  knows when the node is supposed to be alive. It also provides a locking
  construct so that other start/stop operations don't run concurrently with its
  critical section.
...
(defmacro locking
  "Locks a Watchdog for the duration of body. Use this around code that
  starts/stops the server, to avoid race conditions where you e.g. kill the
  server and the watchdog immediately restarts it."
  [watchdog & body]
...
(defn db
  "Wraps an existing database in a one with a watchdog. Takes a map of partial
  options to `watchdog` (just :running? and :interval), and a DB to wrap. Uses
  `db/start!` to start the database. Ensures that db/kill! and db/start!
  disable and enable the watchdog."
  [opts db]
  (map->DB
    {:db        db
     :watchdogs (atom {})
